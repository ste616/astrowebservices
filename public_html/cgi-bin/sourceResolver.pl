...
for (my $i = 0; $i <= $#re;$ i++){
  if ($re[$i] =~ /^\%J\s/) {
    ...
    $name =~ s/%I.0\s+//;
    $name =~ s/NAME\s+//;
  } elsif ($re[$i] =~ /\*\*\* Nothing found \*\*\*/) {
      # No source found with that name.
      print " name: '".$input{'name'}."', position: { ra: undefined,".
	  " dec: undefined, epoch: undefined }, resolver: '', resolvedName: '',".
	  " error: 'No source was found with that name.' }\n";
      exit;
  }
}
